...
PROC MIXED DATA = group_output_df COVTEST NOCLPRINT NAMELEN = 100 IC METHOD = REML;
	...
	MODEL WDRAW_ORIG_YR2_RT = SY_ind / SOLUTION DDFM = SATTERTHWAITE;
...
PROC MIXED DATA = group_output_df COVTEST NOCLPRINT NAMELEN = 100 IC METHOD = REML;
	CLASS school_sid SY_ind;
	MODEL WDRAW_ORIG_YR2_RT = AVGSTUDSENT time1 time1 * time1 / SOLUTION DDFM = SATTERTHWAITE;
	RANDOM INTERCEPT time1 time1 * time1 / G GCORR V VCORR TYPE = UN SUBJECT = school_sid;
	REPEATED SY_ind / R TYPE = VC SUBJECT = school_sid;
	ODS OUTPUT InfoCrit = FitRandQuad;
run; TITLE1;
...
PROC MIXED DATA = group_output_df COVTEST NOCLPRINT NAMELEN = 100 IC METHOD = REML;
	CLASS school_sid SY_ind;
	MODEL WDRAW_ORIG_YR2_RT = AVGSTUDSENT time1 / SOLUTION DDFM = SATTERTHWAITE;
	RANDOM INTERCEPT time1 / G GCORR V VCORR TYPE = UN SUBJECT = school_sid;
	REPEATED SY_ind / R TYPE = VC SUBJECT = school_sid;
	ODS OUTPUT InfoCrit = FitRandQuad;
run; TITLE1;
...
proc glm 
		...
		noprint;
	class school_sid;
	model WDRAW_ORIG_YR2_RT = time1  time1 * time1;
	output 
		out = new_group_output_df
		r = resid;
run; quit;

proc sort data = new_group_output_df; 
	by school_sid;
run;

proc transpose 
		data = new_group_output_df
		out = new_group_output_df_wide
		prefix = resid;
    by school_sid;
	id time1;
	var resid;
...
/* SCATTERPLOT MATRIX */
proc sgscatter 
		data=new_group_output_df_wide;
	title "Residual Scatterplot Matrix";
	matrix resid0 resid8 resid16; /* resid1 resid2 resid3 resid4 resid5 resid6 resid7 resid8 resid9 resid10 resid11 resid12 resid13 resid14 resid15 resid16 */
run; TITLE;
/* RESIDUAL CORRELATION MATRIX */
proc corr
		data = new_group_output_df_wide;
	title 'Residual Correlation Matrix';
	var resid0 resid8 resid16; /* resid1 resid2 resid3 resid4 resid5 resid6 resid7 resid8 resid9 resid10 resid11 resid12 resid13 resid14 resid15 resid16 */
run; TITLE;
...
proc glm 
		...
		noprint;
	class school_sid;
	model WDRAW_ORIG_YR2_RT = time1 AVGSTUDSENT time1 * time1;
	output 
		out = new_group_output_df
		r = resid;
run; quit;
...
proc glm 
		...
		noprint;
	class school_sid;
	model WDRAW_ORIG_YR2_RT = time1 AVGSTUDSENT;
	output 
		out = new_group_output_df
		r = resid;
run; quit;
