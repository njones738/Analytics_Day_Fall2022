...
PROC MIXED DATA = group_output_df COVTEST NOCLPRINT NAMELEN = 100 IC METHOD = REML;
	...
	MODEL WDRAW_ORIG_YR2_RT = SY_ind AVGSTUDSENT / SOLUTION DDFM = SATTERTHWAITE;
...
PROC MIXED DATA = group_output_df COVTEST NOCLPRINT NAMELEN = 100 IC METHOD = REML;
	CLASS school_sid SY_ind;
	MODEL WDRAW_ORIG_YR2_RT = AVGSTUDSENT time1 time1 * time1 / SOLUTION DDFM = SATTERTHWAITE;
	RANDOM INTERCEPT AVGSTUDSENT time1 time1 * time1 / G GCORR V VCORR TYPE = UN SUBJECT = school_sid;
	REPEATED SY_ind / R TYPE = VC SUBJECT = school_sid;
...
proc glm 
		...
		noprint;
	class school_sid;
	model WDRAW_ORIG_YR2_RT = AVGSTUDSENT time1  time1 * time1;
	output 
